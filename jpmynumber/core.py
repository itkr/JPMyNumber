...
class _ValidationMixin(object):
    ...
    def validate_pattern(self):
        if not self.check_pattern(self.get_pattern()):
...
class _CreationMixin(object):
    ...
    def _update_pattern(self):
        arr = self._to_a
        for i, pattern_n in enumerate(self.get_pattern()):
...
class JPMyNumber(_CreationMixin, _ValidationMixin):
    ...
    def get_pattern(self):
        return [None for _i in range(self.LEN)]

    def check_pattern(self, pattern):
        for i, n in enumerate(self._to_a):
            if pattern[i] is None:
                continue
            if hasattr(pattern[i], '__iter__'):
                if n not in pattern[i]:
                    return False
            elif pattern[i] != n:
                return False
        return True
